...
import { DotButton, FilesLoaderService, calculateButtonPrice, DotPrefixes, DotPrefixesLoader, DotModifier, DotButtonType, ComboStepType } from 'dotsdk';
...
import { PrefixID } from '../../enums/prefix-id.enum';
...
import { CatalogPicturePipe } from '../../pipes/catalog-picture.pipe';
import { TranslateCatalogTitle } from '../../pipes/translate-catalog-title.pipe';
...
export class ProductCardComponent implements OnInit, OnDestroy {
  ...
  public get price(): number {
    ...
    if (this.isLinkToParent) {
      return this.childButtons.reduce((acc, button) => acc + this.buttonPrice(button) * button.quantity, basketButtonPrice * this.basketButton.quantity);
  ...
  public get parentButton(): DotButton {
    const comboCatalog = FilesLoaderService.getInstance().loadersMap.get(CombosCatalog) as CombosCatalog;
    return comboCatalog.Buttons.find(btn => btn.Link === this.basketButton.ComboPage.ID.toString());
  }

  public get subtitle(): string {
    const upsizeButton = this.basketButton.ComboPage.Combos
      .find(combo => combo.ComboStepType.toLowerCase() === ComboStepType.ComboUpSize.toLowerCase())?.Buttons.find(btn => btn.Selected);
    return this.dotTranslateCaption.transform(upsizeButton) ?? '';
  }

  public get comboSelectedButtons(): DotButton[] {
    return this.basketButton.ComboPage.Combos
      .filter(combo => combo.Visible && combo.ComboStepType.toLowerCase() === ComboStepType.ComboStep.toLowerCase()).reduce((acc, c) => {
  ...
  public get childButtons(): DotButton[] {
    return this.basketButton['promoChildButtons'] ?
           ...
           this.basketService.buttons.filter(b => this.basketButton['parentLinkUUID'] && b['childLinkUUID'] === this.basketButton['parentLinkUUID']);
  }
  ...
  public defaultQuantityLabel(button: DotButton) {
    ...
    if (button.DefaultQuantity < button.quantity) {
      ...
      if (button.DefaultQuantity > button.quantity && button.quantity > 0)  {
        ...
        return `${this.dotTranslatePipe.transform('2021050701')} ${this.dotTranslateCaption.transform(button)}`;
      }
    }
  }
  ...
  public getSelectedComboModifiers(button: DotButton): string {
    ...
    button?.ModifiersPage?.Modifiers.forEach((modifier) => {
        modifier.selectedButtons.forEach((selectedBtn) => {
          ...
          if (selectedBtn.quantity > 1 && selectedBtn.DefaultQuantity === 0) {
  ...
  public displayChildChange(child: DotButton): boolean {
    return child.isPromo && this.skipPrecalculate &&
  ...
  public getChildModifierButtons(childButton: DotButton): DotButton[] {
    return childButton?.ModifiersPage?.Modifiers?.reduce((acc, mod) => {
  ...
  private removeButton(button: DotButton) {
    if (button.isPromo && !!button['Promo']) {
      ...
      this.basketService.buttons.filter(btn => btn['Promo']?.UUID === promoUUID).forEach(b => this.basketService.removeButtonFromBasket(b));
    } else {
